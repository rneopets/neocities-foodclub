...
function calculateCombination(a, b, c, d, e) {
  ...
  if (vm.displayMode === "custom") {
    ...
    var p = [a, b, c, d, e];
    for (var i = 0; i < p.length; i++) if (p[i] !== 0) prob_denominator *= 100;
  } else {
    odds = vm.currentOdds;
  ...
  if (winnings > 1000000) winnings = 1000000;
  winchance =
    (probability[0][a] *
      probability[1][b] *
      probability[2][c] *
...
function calc_maxter() {
  ...
  for (bet = 0; bet < maxBets; bet++) {
    ...
    for (arena = 0; arena < 5; arena++) {
      ...
      if (old_page) check_radio(arena, pirate, bet);
      else result += pirate;
    }
  }
  if (old_page) return;
  ...
  for (i = 0; i < result.length; i += 2) {
    linkCode += String.fromCharCode(
      97 + parseInt(result.substring(i, i + 2), 5)
    );
  }
  // Reload the page with the new bets
  window.location =
    window.location.href + "#round=" + parseInt(vm.round) + "&b=" + linkCode;
}
